...
pub struct CSegListChan<T: Line> {
    pub lst: Vec<StrongHandle<CSeg<T>>>,
    pub lv: SegType,
...
impl<T: Line> CSegListChan<T> {
    ...
    // 已完备
    pub fn update(&mut self, bi_lst: &[WeakHandle<T>]) {
        self.do_init();
        if self.lst.is_empty() {
    ...
    // 已完备
    fn cal_seg_sure(&mut self, bi_lst: &[WeakHandle<T>], begin_idx: usize) {
    ...
    where
        I: Iterator<Item = &'a WeakHandle<T>>,
        T: 'a, // Add this bound to ensure T lives at least as long as 'a
    {
        ...
        for bi in bi_list.skip(begin_idx) {
            ...
            match (bi.upgrade().unwrap().borrow().line_dir(), last_seg_dir) {
            ...
            if self.lst.is_empty() {
                ...
                if up_eigen.ele[1].is_some() && bi.upgrade().unwrap().borrow().line_is_down() {
                    last_seg_dir = Some(BiDir::Down);
                    down_eigen.clear();
                } else if down_eigen.ele[1].is_some() && bi.upgrade().unwrap().borrow().line_is_up()
                {
                    up_eigen.clear();
                ...
                if up_eigen.ele[1].is_none()
                    && last_seg_dir == Some(BiDir::Down)
                    && bi.upgrade().unwrap().borrow().line_dir() == BiDir::Down
                {
                    last_seg_dir = None;
                } else if down_eigen.ele[1].is_none()
                    && last_seg_dir == Some(BiDir::Up)
                    && bi.upgrade().unwrap().borrow().line_dir() == BiDir::Up
                {
                    last_seg_dir = None;
                }
            }
    ...
    // 已完备
    fn treat_fx_eigen(&mut self, mut fx_eigen: CEigenFX<T>, bi_lst: &[WeakHandle<T>]) {
...
impl<T: Line> CSegListChan<T> {
    ...
    // 已完备
    pub fn collect_first_seg(&mut self, bi_lst: &[WeakHandle<T>]) {
        if bi_lst.len() < 3 {
            return;
        }
        match self.config.left_method {
            LeftSegMethod::Peak => {
                let _high = bi_lst
                    .iter()
                    .map(|bi| bi.upgrade().unwrap().borrow().line_high())
                    .fold(f64::MIN, f64::max);
                let _low = bi_lst
                    .iter()
                    .map(|bi| bi.upgrade().unwrap().borrow().line_low())
                    .fold(f64::MAX, f64::min);
                let first_val = bi_lst
                    .first()
                    .unwrap()
                    .upgrade()
                    .unwrap()
                ...
                if (_high - first_val).abs() >= (_low - first_val).abs() {
                    ...
                    if let Some(peak_bi) = find_peak_bi(bi_lst.iter(), true) {
                        self.add_new_seg(
                            bi_lst,
                            peak_bi.upgrade().unwrap().borrow().line_idx(),
                            false,
                            Some(BiDir::Up),
                            false,
                            "0seg_find_high",
            ...
            LeftSegMethod::All => {
                let _dir = if bi_lst
                    ...
                    .borrow()
                    .line_get_end_val()
                    >= bi_lst[0].upgrade().unwrap().borrow().line_get_begin_val()
                {
                    BiDir::Up
                } else {
                    BiDir::Down
    ...
    pub fn collect_left_seg_peak_method(
        ...
        if last_seg_end_bi.upgrade().unwrap().borrow().line_is_down() {
            if let Some(peak_bi) = find_peak_bi(
                ...
                if peak_bi.upgrade().unwrap().borrow().line_idx()
    ...
    // 99% 已完备，理由同上
    pub fn collect_segs(&mut self, bi_lst: &[WeakHandle<T>]) {
    ...
    // 已完备
    pub fn collect_left_seg(&mut self, bi_lst: &[WeakHandle<T>]) {
        if self.lst.is_empty() {
    ...
    // 已完备
    pub fn collect_left_as_seg(&mut self, bi_lst: &[WeakHandle<T>]) {
        let last_bi = bi_lst.last().unwrap();
        let last_seg_end_bi = self.lst.last().unwrap().borrow().end_bi.clone();
            ...
            == last_bi.upgrade().unwrap().borrow().line_dir()
        {
            ...
            self.add_new_seg(
                bi_lst,
                last_bi.upgrade().unwrap().borrow().line_idx(),
                false,
                None,
                true,
                "collect_left_0",
    ...
    // 实 iter() 方法返不可变迭代器
    pub fn iter(&self) -> Iter<'_, StrongHandle<CSeg<T>>> {
        self.lst.iter()
    }

    // 实现 iter_mut() 方法返回可变迭代器
    pub fn iter_mut(&mut self) -> IterMut<'_, StrongHandle<CSeg<T>>> {
        self.lst.iter_mut()
    }

    // last() 方法已经通过 Vec 的方法自动获得
    pub fn last(&self) -> Option<&StrongHandle<CSeg<T>>> {
        self.lst.last()
    }

    // 如果需要可变的 last()
    pub fn last_mut(&mut self) -> Option<&mut StrongHandle<CSeg<T>>> {
        self.lst.last_mut()
    }
}
    ...
    pub fn try_add_new_seg(
        ...
        if self.lst.is_empty() && split_first_seg && end_bi_idx >= 3 {
            if let Some(peak_bi) = find_peak_bi(
                ...
                let peak_bi_strong = peak_bi.upgrade().unwrap();
                let peak_bi_ref = peak_bi_strong.borrow();
                if (peak_bi_ref.line_is_down()
                    && (peak_bi_ref.line_low() < bi_lst[0].upgrade().unwrap().borrow().line_low()
                        || peak_bi_ref.line_idx() == 0))
                    || (peak_bi_ref.line_is_up()
                        && (peak_bi_ref.line_high()
                            > bi_lst[0].upgrade().unwrap().borrow().line_high()
                            || peak_bi_ref.line_idx() == 0))
                {
                    // 要比第一笔开头还高/低（因为没有比较到）
                    self.add_new_seg(
...
pub fn find_peak_bi<'a, T: Line + 'a, I>(bi_lst: I, is_high: bool) -> Option<WeakHandle<T>>
where
    I: Iterator<Item = &'a WeakHandle<T>>,
{
...
impl<T: Line> Index<usize> for CSegListChan<T> {
    type Output = StrongHandle<CSeg<T>>;
...
impl<'a, T: Line> IntoIterator for &'a CSegListChan<T> {
    type Item = &'a StrongHandle<CSeg<T>>;
    type IntoIter = Iter<'a, StrongHandle<CSeg<T>>>;
...
impl<'a, T: Line> IntoIterator for &'a mut CSegListChan<T> {
    type Item = &'a mut StrongHandle<CSeg<T>>;
    type IntoIter = IterMut<'a, StrongHandle<CSeg<T>>>;
