...
pub struct CEigenFX {
    pub lv: SegType,
    pub dir: BiDir,
    pub ele: [Option<Handle<CEigen>>; 3],
    pub lst: Vec<Handle<CBi>>,
    pub exclude_included: bool,
    pub kl_dir: KlineDir,
    pub last_evidence_bi: Option<Handle<CBi>>,
}
...
impl CEigenFX {
    ...
    fn treat_first_ele(&mut self, bi: Handle<CBi>) -> bool {
        self.ele[0] = Some(Rc::new(RefCell::new(CEigen::new(bi, self.kl_dir))));
        false
    }

    fn treat_second_ele(&mut self, bi: Handle<CBi>) -> bool {
    ...
    fn treat_third_ele(&mut self, bi: Handle<CBi>) -> bool {
    ...
    pub fn add(&mut self, bi: Handle<CBi>) -> bool {
    ...
    fn find_revert_fx(
        ...
        let mut eigen_fx = CEigenFX::new(revert_bi_dir(first_BiDir), !COMMON_COMBINE, self.lv);
