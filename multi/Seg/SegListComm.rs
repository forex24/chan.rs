...
pub struct CSegListComm<SUB_LINE_TYPE> {
    pub lst: Vec<Handle<CSeg<SUB_LINE_TYPE>>>,
    pub lv: SegType,
...
impl<SUB_LINE_TYPE> CSegListComm<SUB_LINE_TYPE> {
    ...
    pub fn collect_left_seg_peak_method(
        &mut self,
        last_seg_end_bi: Handle<SUB_LINE_TYPE>,
        bi_lst: &CBiList,
...
impl<SUB_LINE_TYPE> std::ops::Index<usize> for CSegListComm<SUB_LINE_TYPE> {
    type Output = Handle<CSeg<SUB_LINE_TYPE>>;
...
impl<SUB_LINE_TYPE> std::ops::Deref for CSegListComm<SUB_LINE_TYPE> {
    type Target = Vec<Handle<CSeg<SUB_LINE_TYPE>>>;
...
pub fn find_peak_bi(bi_lst: &[Handle<CBi>], is_high: bool) -> Option<Handle<CBi>> {
