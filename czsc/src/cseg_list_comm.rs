use crate::{
    CSeg, CSegListChan, Direction, Handle, IParent, LeftSegMethod, LineType, TestSegError, ToHandle,
};
...
impl<T: LineType + IParent + ToHandle> CSegListChan<T> {
    ...
    /// # Arguments
    /// * `bi_lst` - 笔列表
    pub(crate) fn collect_first_seg(&mut self, bi_lst: &[T]) {
        if bi_lst.len() < 3 {
            return;
        }
    ...
    /// # Arguments
    /// * `bi_lst` - 笔列表
    pub(crate) fn collect_segs(&mut self, bi_lst: &[T]) {
    ...
    /// # Arguments
    /// * `bi_lst` - 笔列表
    pub(crate) fn collect_left_seg(&mut self, bi_lst: &[T]) {
    ...
    /// # Arguments
    /// * `bi_lst` - 笔列表
    pub(crate) fn collect_left_as_seg(&mut self, bi_lst: &[T]) {
        let last_bi = bi_lst.last().unwrap();
        let last_seg_end_bi = self.lst.last().unwrap().end_bi;
    ...
    pub(crate) fn check_seg_valid(&self, seg: &CSeg<T>) -> Result<(), TestSegError> {
        if !seg.is_sure {
            return Ok(());
        }
